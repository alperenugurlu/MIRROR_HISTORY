...
import type { ColumnMapping, ImportResult, CsvPreviewRow } from '../../shared/types';
  ...
  const insertMany = dbInstance.transaction(() => {
    for (const row of result.data) {
      try {
        ...
        // Create event for the transaction
        ...
        if (tx) {
          // Store evidence linking back to CSV row
          db.insertEvidence(
            event.id,
            'csv_row',
            filePath,
...
function normalizeMerchant(raw: string): string {
  return raw
    .replace(/\s+/g, ' ')
    .replace(/\s*[-#]\s*\d+.*$/, '') // strip trailing reference numbers
    .trim();
}
