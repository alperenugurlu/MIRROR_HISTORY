...
export function generateDiff(periodType: PeriodType, refDate: string): DiffResult {
  ...
  broadcast('diff_generated', {
    diff_id: storedDiff.id,
    period_type: periodType,
    summary: diffSummary,
    card_count: filteredCards.length,
  });
...
function createDetectionEvent(
  ...
  const merchant = (details as any).merchant || '';
  const contentHash = db.computeContentHash([type, merchant, periodStart, periodEnd]);
